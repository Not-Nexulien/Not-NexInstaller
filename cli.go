...
func main() {
	...
	var installFlag = flag.Bool("install", false, "Install Not-Nexulien")
	var updateFlag = flag.Bool("repair", false, "Repair Not-Nexulien")
	var uninstallFlag = flag.Bool("uninstall", false, "Uninstall Not-Nexulien")
	...
	if *versionFlag {
		fmt.Println("Not-Nexulien Installer Cli", buildinfo.InstallerTag, "("+buildinfo.InstallerGitHash+")")
	...
	if !SliceContainsFunc(switches, func(b *bool) bool { return *b }) {
		...
		go func() {
			...
			if IsSelfOutdated {
				...
				Log.Warn("To update, select the 'Update Not-Nexulien Installer' option to update, or run with --update-self")
			}
		}()

		choices := []string{
			"Install Not-Nexulien",
			"Repair Not-Nexulien",
			"Uninstall Not-Nexulien",
			"Install OpenAsar",
			"Uninstall OpenAsar",
			"View Help Menu",
			"Update Not-Nexulien Installer",
			"Quit",
		}
		...
		case "Quit":
			return
		case "Update Not-Nexulien Installer":
			if err := UpdateSelf(); err != nil {
				Log.Error("Failed to update self:", err)
				exitFailure()
			}
	...
	if install {
		...
		Log.Info("Downloading latest Not-Nexulien files...")
...
func HandleScuffedInstall() {
	...
	fmt.Println("Otherwise, Not-Nexulien will likely not work.")
}
